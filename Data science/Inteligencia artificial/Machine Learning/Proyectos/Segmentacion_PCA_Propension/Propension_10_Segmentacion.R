...
# library(zoo)
library(plyr)
# Paquetes analisis multivariado
library("FactoMineR")
#install.packages("factoextra")
library("factoextra")
library(ca)
library(klaR) # Clasificacion por kmodes
library(cluster) # graficos cluster
#library(modeest) # Clasificacion por kmodes
...
# b <-SISMAR %>% filter(NIT_EMP_COL %in%  c('1152205168'))
...
for (i in categ) {
  ...
  tmp <- as.data.frame(SISMAR[[i]] %>% table()) %>% 
            arrange( desc(Freq))
  tmp <- tmp %>%   
    filter(!(. %in%  c('0',''))) %>%
    ...
    filter(if (all_of(i) != "TIPO_CARGA") 
      aporte >= 0.01 | is.na(aporte) else TRUE) %>% 
    head(10) 
  colnames(tmp) <- c(i,'Freq','cumSum','percent','aporte')
  ...
  SISMAR[[i]] <- ifelse(SISMAR[[i]] %in% tmp[[i]],
                        SISMAR[[i]],
                        ifelse(is.na(SISMAR[[i]]),NA,'OTROS'))
  ...
  transf_tmp <- SISMAR %>% 
    ...
    mutate(valor = ifelse(is.na(i),0,1)) %>% 
  ...
  colnames(transf_tmp) <- paste0(i, "|", colnames(transf_tmp))
...
bd.ca = SISMAR_Segmentacion_CA %>% 
  #dplyr::select(-PUERTO_NIT_EMP_COL, TONELADAS)
...
res.ca = PCA(bd.ca, graph = F)
#res.ca = MFA(bd.ca, graph = F)

# Generamos la segmentacion utilizando la informacion descrita en CA
...
#sismar_hcpc$NIT_EMP_COL <- row.names(sismar_hcpc) 
  

sismar_hcpc <- sismar_hcpc %>% 
...
# Campo que nos trae las variables originales seleccionadas en el modelo
categ <- categ
