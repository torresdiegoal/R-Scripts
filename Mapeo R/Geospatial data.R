...
library(tidyverse)
library(sf) # Simple Features for R, for Spatial vector data
library(googlesheets)
library(rvest)
library(rmapshaper)

# read in geospatial data
ind_sfx <- st_read("D:/Disco D/R/Mapas/shapes/viz_india-master/india_states_2014/india_states.shp")
...
tidy_pop <- pop %>%
    ...
    mutate(
        ...
        density_km2 = sub("/.*", "", density_km2) %>% 
            str_replace_all(",", "") %>%
            as.numeric(),
        state_ut = replace(state_ut, state_ut == 'ManipurÎ²', 'Manipur')
    ) %>% 
    arrange(state_ut)
